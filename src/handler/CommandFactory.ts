...
export class CommandFactory {
  ...
  constructor(commands: Array<new () => any>) {
    commands.forEach((Command) => {
      ...
      Object.getOwnPropertySymbols(Command)
        .concat(Object.getOwnPropertySymbols(cmd))
        .forEach((symbol) => {
          if (cmd[symbol])
            cmd[symbol] = Object.assign(cmd[symbol], Command[symbol])
  ...
  findCommand(name: string): BaseSymbols | undefined {
    return this.commands.find(
      (cmd) =>
        cmd[Symbols.commandName] === name || cmd[Symbols.aliases].includes(name)
    )
  }
}
